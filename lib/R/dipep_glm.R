...
dipep_glm <- function(df              = .data,
    ...
    results$table <- dcast(results$table, levels ~ classification) %>%
                     mutate(predictor = predictor,
                            ...
                            predictor = gsub('wells.dvt', 'Wells : DVT', predicted),
                            predictor = gsub('wells.alternative.permissive', 'Wells : DVT', predicted),
                            predictor = gsub('wells.alternative.strict', 'Wells : DVT', predicted),
                            predictor = gsub('wells.heart.rate', 'Wells : DVT', predicted),
                            predictor = gsub('wells.surgery.immobil', 'Wells : DVT', predicted),
                            predictor = gsub('wells.dvt.pe', 'Wells : DVT', predicted),
                            predictor = gsub('wells.haemoptysis', 'Wells : DVT', predicted),
                            predictor = gsub('wells.neopasm', 'Wells : Neoplasm', predicted),
                            predictor = gsub('wells.dvt', 'Wells : DVT', predicted),
                            predictor = gsub('wells.dvt', 'Wells : DVT', predicted),
                            predictor = gsub('delphi.haemoptysis', 'Delphi : Haemoptysis', predicted),
                            predictor = gsub('delphi.pleuritic', 'Delphi : Pleuritic', predicted),
                            predictor = gsub('delphi.history.dvt.pe', 'Delphi : History DVT/PE', predicted),
                            predictor = gsub('delphi.family.history', 'Delphi : Family History', predicted),
                            predictor = gsub('delphi.medical.history', 'Delphi : Medical History', predicted),
                            predictor = gsub('delphi.obstetric.complication', 'Delphi : Obstetric Complication', predicted),
                            predictor = gsub('delphi.medical.complication', 'Delphi : Medical Complication', predicted),
                            predictor = gsub('delphi.gestation', 'Delphi : Gestation', predicted),
                            predictor = gsub('delphi.bmi', 'Delphi : BMI', predicted),
                            predictor = gsub('delphi.dvt', 'Delphi : DVT', predicted),
                            predictor = gsub('delphi.o2.saturation', 'Delphi : O2 Saturation', predicted),
                            predictor = gsub('delphi.heart.rate', 'Delphi : Heart Rate', predicted),
                            predictor = gsub('delphi.respiratory.rate', 'Delphi : Respiratory Rate', predicted)
                            )
