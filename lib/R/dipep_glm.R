...
dipep_glm <- function(df              = .data,
    ...
    results$table <- dcast(results$table, levels ~ classification) %>%
                     mutate(predictor = predictor,
                            ...
                            predictor = gsub('wells.alternative.permissive', 'Wells : DVT', predictor),
                            predictor = gsub('wells.alternative.strict', 'Wells : DVT', predictor),
                            predictor = gsub('wells.heart.rate', 'Wells : DVT', predictor),
                            predictor = gsub('wells.surgery.immobil', 'Wells : DVT', predictor),
                            predictor = gsub('wells.dvt.pe', 'Wells : DVT', predictor),
                            predictor = gsub('wells.haemoptysis', 'Wells : DVT', predictor),
                            predictor = gsub('wells.neopasm', 'Wells : Neoplasm', predictor),
                            predictor = gsub('wells.dvt', 'Wells : DVT', predictor),
                            predictor = gsub('wells.dvt', 'Wells : DVT', predictor),
    ...
    results$tidied <- mutate(results$tidied,
                             ...
                             term = gsub('\\.cat', ' ', term),
                             term = gsub('simplified.Age (Continuous)', 'Geneva : Age > 35', term),
                             ...
                             term = gsub('simplified.Heart Rate (Continuous)', 'Geneva : Heart Rate', term),
