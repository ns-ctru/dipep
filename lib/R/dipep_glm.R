...
dipep_glm <- function(df              = .data,
    ...
    results$table <- dcast(results$table, levels ~ classification) %>%
                     mutate(predictor = predictor,
                            ...
                            predictor = gsub('medical.complication', 'Medical Complication (Delphi)', predictor),
                            predictor = gsub('obstetric.complication', 'Obstetric Complication (Delphi)', predictor),
    ...
    results$tidied <- mutate(results$tidied,
                             ...
                             term = gsub('medical.complicationYes', 'Medical Complication (Delphi)', term),
                             term = gsub('medical.complicationNo', 'Medical Complication (Delphi)', term),
                             term = gsub('obstetric.complicationYes', 'Obstetric Complication (Delphi)', term),
                             term = gsub('obstetric.complicationNo', 'Obstetric Complication (Delphi)', term),
                             term = gsub('pregnancies.under.catYes', '>= 1 Pregnancy < 24 weeks', term),
                             term = gsub('pregnancies.under.catNo', '>= 1 Pregnancy < 24 weeks', term),
                             term = gsub('pregnancies.over.catYes', '>= 1 Pregnancy > 24 weeks', term),
                             term = gsub('pregnancies.over.catNo', '>= 1 Pregnancy > 24 weeks', term),
