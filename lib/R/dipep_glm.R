...
dipep_glm <- function(df              = .data,
    ...
    results$table <- dcast(results$table, levels ~ classification) %>%
                  ...
                  ## case_when(## .$predictor == 'simplified.age' ~ 'Geneva : Age',
                            ## .$predictor == 'simplified.prev.dvt.pe' ~ 'Geneva : Previous DVT/PE',
                            ## .$predictor == 'simplified.surgery' ~ 'Geneva : Surgery',
                            ## .$predictor == 'simplified.neoplasm' ~ 'Geneva : Neoplasm',
                            ## .$predictor == 'simplified.lower.limb.unilateral.pain' ~ 'Geneva : Unilateral Lower Limb Pain',
                            ## .$predictor == 'simplified.haemoptysis' ~ 'Geneva : Haemoptysis',
                            ## .$predictor == 'simplified.heart.rate' ~ 'Geneva : Heart Rate',
                            ## .$predictor == 'simplified.pain.palpitations' ~ 'Geneva : Pain on limb palpitation',
                            ## .$predictor == 'perc.age' ~ 'PERC : Age',
                            ## .$predictor == 'perc.heart.rate' ~ 'PERC : Heart Rate',
                            ## .$predictor == 'perc.o2.saturation' ~ 'PERC : O2 Saturation',
                            ## .$predictor == 'perc.prev.dvt.pe' ~ 'PERC : Previous DVT/PE',
                            ## .$predictor == 'perc.surgery' ~ 'PERC : Recent Trauma or Surgery',
                            ## .$predictor == 'perc.haemoptysis' ~ 'PERC : Haemoptysis',
                            ## .$predictor == 'perc.hormone' ~ 'PERC : Exogenous Estrogen',
                            ## .$predictor == 'perc.leg.swelling' ~ 'PERC : Unilateral Leg Swelling',
                            ## .$predictor == 'wells.dvt' ~ 'Wells : DVT',
                            ## .$predictor == 'wells.alternative.permissive' ~ 'Wells : PE most likely (Permissive)',
                            ## .$predictor == 'wells.alternative.strict' ~ 'Wells : PE most likely (Strict)',
                            ## .$predictor == 'wells.heart.rate' ~ 'Wells : Heart Rate',
                            ## .$predictor == 'wells.surgery.immobil' ~ 'Wells : Surgery/Immobility',
                            ## .$predictor == 'wells.dvt.pe' ~ 'Wells : Previous DVT/PE',
                            ## .$predictor == 'wells.haemoptysis' ~ 'Wells : Haemoptysis',
                            ## .$predictor == 'wells.neoplasm' ~ 'Wells : Neoplasm',
                            ## .$predictor == 'delphi.haemoptysis' ~ 'Delphi : Haemoptysis',
                            ## .$predictor == 'delphi.pleuritic' ~ 'Delphi : Pleuritic',
                            ## .$predictor == 'delphi.history.dvt.pe' ~ 'Delphi : History DVT/PE',
                            ## .$predictor == 'delphi.family.history' ~ 'Delphi : Family History',
                            ## .$predictor == 'delphi.medical.history' ~ 'Delphi : Medical History  (Hospital Admission, Surgery or Injury)',
                            ## .$predictor == 'delphi.medical.history.surgery' ~ 'Delphi : Surgery (excl. Cesarean)',
                            ## .$predictor == 'delphi.obstetric.complication' ~ 'Delphi : Obstetric Complication',
                            ## .$predictor == 'delphi.medical.complication' ~ 'Delphi : Medical Complication',
                            ## .$predictor == 'delphi.gestation' ~ 'Delphi : Gestation',
                            ## .$predictor == 'delphi.bmi' ~ 'Delphi : BMI',
                            ## .$predictor == 'delphi.dvt' ~ 'Delphi : DVT',
                            ## .$predictor == 'delphi.o2.saturation' ~ 'Delphi : O2 Saturation',
                            ## .$predictor == 'delphi.heart.rate' ~ 'Delphi : Heart Rate',
                            ## .$predictor == 'delphi.respiratory.rate' ~ 'Delphi : Respiratory Rate',
                            ## .$predictor == 'age.cat' ~ 'Age (Categorised)',
                            ## .$predictor == 'smoking' ~ 'Smoking Status',
                            ## .$predictor == 'injury' ~ 'Injury',
                            ## .$predictor == 'temperature.cat' ~ 'Temperature (Categorised)',
                            ## .$predictor == 'bp.diastolic.cat' ~ 'Diastolic (Categorised)',
                            ## .$predictor == 'bp.systolic.cat' ~ 'Systolic (Categorised)',
                            ## .$predictor == 'o2.saturation.cat' ~ 'O2 Saturation (Categorised)',
                            ## .$predictor == 'respiratory.rate.cat' ~ 'Respiratory Rate (Categorised)',
                            ## .$predictor == 'heart.rate.cat' ~ 'Heart Rate (Categorised)',
                            ## .$predictor == 'bmi.cat' ~ 'BMI (Categorised)',
                            ## .$predictor == 'presenting.features.pleuritic' ~ 'Presenting : Pleuritic',
                            ## .$predictor == 'presenting.features.non.pleuritic' ~ 'Presenting : Non-Pleuritic',
                            ## .$predictor == 'presenting.features.sob.exertion' ~ 'Presenting : Shortness of Breath (Exertion)',
                            ## .$predictor == 'presenting.features.sob.rest' ~ 'Presenting : Shortness of Breath (Rest)',
                            ## .$predictor == 'presenting.features.haemoptysis' ~ 'Presenting : Haemoptysis',
                            ## .$predictor == 'presenting.features.cough' ~ 'Presenting : Cough',
                            ## .$predictor == 'presenting.features.syncope' ~ 'Presenting : Syncope',
                            ## .$predictor == 'presenting.features.palpitations' ~ 'Presenting : Palpitations',
                            ## .$predictor == 'presenting.features.other' ~ 'Presenting : Other',
                            ## .$predictor == 'pregnancies.under.cat' ~ '>= 1 Pregnancy < 24 weeks',
                            ## .$predictor == 'pregnancies.over.cat' ~ '>= 1 Pregnancy > 24 weeks',
                            ## .$predictor == 'prev.preg.problem' ~ 'Previous Pregnancy Problems',
                            ## .$predictor == 'medical.probs' ~ 'Other Previous Medical Problesm',
                            ## .$predictor == 'history.thrombosis' ~ 'Family History of Thrombosis',
                            ## .$predictor == 'history.veins' ~ 'History of Varicose Veins',
                            ## .$predictor == 'history.iv.drug' ~ 'History of IV Drug use',
                            ## .$predictor == 'thrombosis' ~ 'History of Thrombosis',
                            ## .$predictor == 'trimester' ~ 'Trimester',
                            ## .$predictor == 'this.pregnancy.problems.incl.other' ~ 'Problems with this Pregnancy (Including Other)',
                            ## .$predictor == 'this.pregnancy.problems' ~ 'Problems with this Pregnancy',
                            ## .$predictor == 'surgery' ~ 'Surgery in previous 4 weeks',
                            ## .$predictor == 'thrombo' ~ 'Known Thrombophilia',
                            ## .$predictor == 'multiple.preg' ~ 'Multiple Pregnancy',
                            ## .$predictor == 'travel' ~ 'Long-haul travel during pregnancy',
                            ## .$predictor == 'immobil' ~ '<3 days Immobility/bed rest during pregnancy',
                            ## .$predictor == 'dvt.cat' ~ 'Clinical DVT',
                            ## .$predictor == 'thromb.event' ~ 'Previous Thrombotic Event',
                            ## .$predictor == 'ecg.pe' ~ 'ECG (PE Related)',
                            ## .$predictor == 'ecg' ~ 'ECG',
                            ## .$predictor == 'xray.pe' ~ 'X-ray (PE Related)',
                            ## .$predictor == 'xray' ~ 'X-ray',
                            ## .$predictor == 'aptt' ~ 'APTT',
                            ## .$predictor == 'aptt.cat' ~ 'APTT : Dichotomised',
                            ## .$predictor == 'prothombin.time' ~ 'Prothombin Time',
                            ## .$predictor == 'prothombin.time.cat' ~ 'Prothombin Time : Dichotomised',
                            ## .$predictor == 'clauss.fibrinogen' ~ 'Clauss Fibrinogen',
                            ## .$predictor == 'clauss.fibrinogen.cat' ~ 'Clauss Fibrinogen : Dichotomised',
                            ## .$predictor == 'ddimer.innovan' ~ 'D-Dimer (Innovan)',
                            ## .$predictor == 'ddimer.innovan.cat' ~ 'D-Dimer (Innovan) : Dichotomised',
                            ## .$predictor == 'ddimer.elisa' ~ 'D-Dimer (ELISA)',
                            ## .$predictor == 'ddimer.elisa.cat' ~ 'D-Dimer (ELISA) : Dichotomised',
                            ## .$predictor == 'd.dimer.cat' ~ 'D-Dimer (Hospital) : Dichotomised',
                            ## .$predictor == 'd.dimer.gestation.cat' ~ 'D-Dimer (Hospital) : Dichotomised (Gestation Specific)',
                            ## .$predictor == 'd.dimer' ~ 'D-Dimer (Hospital)',
                            ## .$predictor == 'thrombin.generation.lag.time' ~ 'Thrombin Generation (Lag Time)',
                            ## .$predictor == 'thrombin.generation.lag.time.cat' ~ 'Thrombin Generation (Lag Time) : Dichotomised',
                            ## .$predictor == 'thrombin.generation.endogenous.potential' ~ 'Thrombin Generation (Endogenous Potential)',
                            ## .$predictor == 'thrombin.generation.endogenous.potential.cat' ~ 'Thrombin Generation (Endogenous Potential) : Dichotomised',
                            ## .$predictor == 'thrombin.generation.peak' ~ 'Thrombin Generation (Peak)',
                            ## .$predictor == 'thrombin.generation.peak.cat' ~ 'Thrombin Generation (Peak) : Dichotomised',
                            ## .$predictor == 'thrombin.generation.time.to.peak' ~ 'Thrombin Generation (Time to Peak)',
                            ## .$predictor == 'thrombin.generation.time.to.peak.cat' ~ 'Thrombin Generation (Time to Peak) : Dichotomised',
                            ## .$predictor == 'plasmin.antiplasmin' ~ 'Plasmin-antiplasmin',
                            ## .$predictor == 'plasmin.antiplasmin.cat' ~ 'Plasmin-antiplasmin : Dichotomised',
                            ## .$predictor == 'prothrombin.fragments' ~ 'Prothombin Fragments',
                            ## .$predictor == 'prothrombin.fragments.cat' ~ 'Prothombin Fragments : Dichotomised',
                            ## .$predictor == 'soluble.tissue.factor' ~ 'Soluble Tissue Factor',
                            ## .$predictor == 'soluble.tissue.factor.cat' ~ 'Soluble Tissue Factor : Dichotomised',
                            ## .$predictor == 'troponin' ~ 'Troponin',
                            ## .$predictor == 'troponin.cat' ~ 'Troponin : Dichotomised',
                            ## .$predictor == 'bnp' ~ 'BNP',
                            ## .$predictor == 'bnp.cat' ~ 'BNP : Dichotomised',
                            ## .$predictor == 'mrproanp' ~ 'MRproANP',
                            ## .$predictor == 'mrproanp.cat' ~ 'MRproANP : Dichotomised',
                            ## .$predictor == 'medical.complication' ~ 'Medical Complication (Delphi reviewed)',
                            ## .$predictor == 'obstetric.complication' ~ 'Obstetric Complication (Delphi reviewed)', predictor),
                            ...
                            ## predictor = gsub('medical.probs', 'Other Previous Medical Problesm', predictor),
