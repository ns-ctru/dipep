...
master$biomarker_tidy <- master$biomarker_tidy %>%
    mutate(prothombin.time.cat = ifelse(prothombin.time > 15.9,
                                        yes = 'Abnormal',
                                        no  = 'Normal'),
           prothombin.time.cat = factor(prothombin.time.cat,
                                        levels = c('Normal', 'Abnormal')),
           prothombin.time.cat = relevel(prothombin.time.cat,
                                         ref = 'Normal'),
           prothrombin.fragments.cat = ifelse(prothrombin.fragments > 1200,
                                              yes = 'Abnormal',
                                              no  = 'Normal'),
           prothrombin.fragments.cat = factor(prothrombin.fragments.cat,
                                              levels = c('Normal', 'Abnormal')),
           prothrombin.fragments.cat = relevel(prothrombin.fragments.cat,
                                               ref = 'Normal'),
           aptt.cat = ifelse(aptt > 52,
                             yes = 'Abnormal',
                             no  = 'Normal'),
           aptt.cat = factor(aptt.cat,
                             levels = c('Normal', 'Abnormal')),
           aptt.cat = relevel(aptt.cat,
                              ref = 'Normal'),
           clauss.fibrinogen.cat = ifelse(clauss.fibrinogen > 4.11,
                                          yes = 'Abnormal',
                                          no  = 'Normal'),
           clauss.fibrinogen.cat = factor(clauss.fibrinogen.cat,
                                          levels = c('Normal', 'Abnormal')),
           clauss.fibrinogen.cat = relevel(clauss.fibrinogen.cat,
                                              ref = 'Normal'),
           thrombin.generation.endogenous.potential.cat = ifelse(thrombin.generation.endogenous.potential > 1533,
                                                                 yes = 'Abnormal',
                                                                 no  = 'Normal'),
           thrombin.generation.endogenous.potential.cat = factor(thrombin.generation.endogenous.potential.cat,
                                                                 levels = c('Normal', 'Abnormal')),
           thrombin.generation.endogenous.potential.cat = relevel(thrombin.generation.endogenous.potential.cat,
                                                                  ref = 'Normal'),
           thrombin.generation.lag.time.cat = ifelse(thrombin.generation.lag.time > 3.4,
                                                     yes = 'Abnormal',
                                                     no  = 'Normal'),
           thrombin.generation.lag.time.cat = factor(thrombin.generation.lag.time.cat,
                                                     levels = c('Normal', 'Abnormal')),
           thrombin.generation.lag.time.cat = relevel(thrombin.generation.lag.time.cat,
                                                      ref = 'Normal'),
           thrombin.generation.time.to.peak.cat = ifelse(thrombin.generation.time.to.peak > 7.7,
                                                         yes = 'Abnormal',
                                                         no  = 'Normal'),
           thrombin.generation.time.to.peak.cat = factor(thrombin.generation.time.to.peak.cat,
                                                         levels = c('Normal', 'Abnormal')),
           thrombin.generation.time.to.peak.cat = relevel(thrombin.generation.time.to.peak.cat,
                                                          ref = 'Normal'),
           thrombin.generation.peak.cat = ifelse(thrombin.generation.peak > 475,
                                                 yes = 'Abnormal',
                                                 no  = 'Normal'),
           thrombin.generation.peak.cat = factor(thrombin.generation.peak.cat,
                                                 levels = c('Normal', 'Abnormal')),
           thrombin.generation.peak.cat = relevel(thrombin.generation.peak.cat,
                                                  ref = 'Normal'),
           ddimer.innovance.cat = ifelse(ddimer.innovance > 1.13,
                                         yes = 'Abnormal',
                                         no  = 'Normal'),
           ddimer.innovance.cat = factor(ddimer.innovance.cat,
                                         levels = c('Normal', 'Abnormal')),
           ddimer.innovance.cat = relevel(ddimer.innovance.cat,
                                          ref = 'Normal'),
           ddimer.elisa.cat = ifelse(ddimer.elisa > 400,
                                     yes = 'Abnormal',
                                     no  = 'Normal'),
           ddimer.elisa.cat = factor(ddimer.elisa.cat,
                                     levels = c('Normal', 'Abnormal')),
           ddimer.elisa.cat = relevel(ddimer.elisa.cat,
                                      ref = 'Normal'),
           plasmin.antiplasmin.cat = ifelse(plasmin.antiplasmin > 800,
                                            yes = 'Abnormal',
                                            no  = 'Normal'),
           plasmin.antiplasmin.cat = factor(plasmin.antiplasmin.cat,
                                            levels = c('Normal', 'Abnormal')),
           plasmin.antiplasmin.cat = relevel(plasmin.antiplasmin.cat,
                                             ref = 'Normal'),
           tissue.factor.cat = ifelse(tissue.factor > 300,
                                      yes = 'Abnormal',
                                      no  = 'Normal'),
           tissue.factor.cat = factor(tissue.factor.cat,
                                      levels = c('Normal', 'Abnormal')),
           tissue.factor.cat = relevel(tissue.factor.cat,
                                       ref = 'Normal'),
           troponin.cat = ifelse(troponin > 2.63,
                                 yes = 'Abnormal',
                                 no  = 'Normal'),
           troponin.cat = factor(troponin.cat,
                                 levels = c('Normal', 'Abnormal')),
           troponin.cat = relevel(troponin.cat,
                                  ref = 'Normal'),
           bnp.cat = ifelse(bnp > 523,
                            yes = 'Abnormal',
                            no  = 'Normal'),
           bnp.cat = factor(bnp.cat,
                            levels = c('Normal', 'Abnormal')),
           bnp.cat = relevel(bnp.cat,
                             ref = 'Normal'),
           mrproanp.cat = ifelse(mrproanp > 954,
                                 yes = 'Abnormal',
                                 no  = 'Normal'),
...
dipep <- dipep %>%
    mutate(bmi = weight / (height / 100)^2,
           ...
           prev.preg.problem = factor(prev.preg.problem,
                                      levels = c('No', 'Yes')),
           ## 2016-10-27 - See email from s.goodacre@sheffield.ac.uk 2016-10-27 @ 10:46
           ## 2017-03-30 - Updating cesarean with information from Delivery.csv
           ...
           hyperemesis = ifelse(is.na(hyperemesis),
                                yes = 0,
                                no  = hyperemesis),
           stillborn = ifelse(is.na(stillborn),
                              yes = 'No',
                              no  = as.character(stillborn)),
...
dipep <- mutate(dipep,
                ...
                dvt      = as.character(dvt),
                dvt.cat  = ifelse(is.na(dvt) | dvt != 'Yes',
                                  yes = 'No',
                                  no  = 'Yes'),
                dvt.cat  = factor(dvt.cat,
                                  labels = c('No', 'Yes')),
                xray     = factor(xray),
                ...
                admitted.hospital    = factor(admitted.hospital,
                                              levels = c('No', 'Yes')),
                d.dimer.cat = factor(d.dimer.cat,
                                     levels = c('Normal', 'Abnormal')),
...
dipep <- mutate(dipep,
                ...
                prev.preg.problem                 = relevel(prev.preg.problem,
                                                            ref = 'No'),
                medical.probs                     = relevel(medical.probs,
                                                            ref = 'No'),
                ...
                thrombosis                        = relevel(thrombosis,
                                                            ref = 'No'),
                thromb.event                      = relevel(thromb.event,
                                                            ref = 'No'),
...
dipep <- dipep %>%
         mutate(## Three forms of the Delphi Consensus score are required.  Start
                ...
                delphi.medical.history.surgery = case_when(is.na(.$surgery)   ~ FALSE,
                                                           .$surgery == 'No'  ~ FALSE,
                                                           .$surgery == 'Yes' & .$preg.post == 'Postpartum' ~ FALSE,
                                                           .$surgery == 'Yes' & .$preg.post == 'Pregnant' ~ TRUE),
                ...
                delphi.obstetric.complication = ifelse(.$hyperemesis == 'Hyperemesis',
                                                       yes = TRUE,
                                                       no  = delphi.obstetric.complication),
                delphi.obstetric.complication = ifelse(.$cesarean == 'Cesarean',
                                                       yes = TRUE,
                                                       no  = delphi.obstetric.complication),
                delphi.obstetric.complication = ifelse(.$stillborn == 'Yes',
                                                       yes = TRUE,
                                                       no  = delphi.obstetric.complication),
                delphi.obstetric.complication = ifelse(.$multiple.preg == 'Yes',
                                                       yes = TRUE,
                                                       no  = delphi.obstetric.complication),
                ...
                ##                              is.na(.$trimester) & .$heart.rate <= 100 ~ FALSE,
                ##                              is.na(.$trimester) & .$heart.rate > 100  ~ TRUE,
...
#######################################################################
dipep <- dipep %>%
         mutate(first.st.vte = case_when(.$group %in% c('Suspected PE', 'Diagnoised PE') & .$first.st == 'PE'    ~ 'VTE',
                                .$group %in% c('Suspected PE', 'Diagnoised PE') & .$first.st == 'No PE' ~ 'No VTE',
                                .$group %in% c('Diagnosed DVT')                                         ~ 'VTE'),
                first.st.vte = factor(first.st.vte,
                                      levels = c('No VTE', 'VTE')),
                second.st.vte = case_when(.$group %in% c('Suspected PE', 'Diagnoised PE') & .$second.st == 'PE'    ~ 'VTE',
                                .$group %in% c('Suspected PE', 'Diagnoised PE') & .$second.st == 'No PE' ~ 'No VTE',
                                .$group %in% c('Diagnosed DVT')                                         ~ 'VTE'),
                second.st.vte = factor(second.st.vte,
                                      levels = c('No VTE', 'VTE')),
                third.st.vte = case_when(.$group %in% c('Suspected PE', 'Diagnoised PE') & .$third.st == 'PE'    ~ 'VTE',
                                .$group %in% c('Suspected PE', 'Diagnoised PE') & .$third.st == 'No PE' ~ 'No VTE',
                                .$group %in% c('Diagnosed DVT')                                         ~ 'VTE'),
                third.st.vte = factor(third.st.vte,
                                      levels = c('No VTE', 'VTE')),
                fourth.st.vte = case_when(.$group %in% c('Suspected PE', 'Diagnoised PE') & .$fourth.st == 'PE'    ~ 'VTE',
...
dipep.README                                                                         <- within(dipep.README,{
                 ...
                 description[data.frame == 'questionnaire.contact']                  <- 'Questionnaire Contact'
                 ...
                 description[data.frame == 'sign.off']                               <- 'Sign Off'
                 ...
                 description[data.frame == 'unavailable.forms']                      <- 'Unavailables'
                 ...
                 description[data.frame == 'annotations']                            <- 'Annotations'
                 description[data.frame == 'service.receipt']                        <- 'Client Service Receipt Inventory'
                 description[data.frame == 'service.receipt.hospital']               <- 'Client Service Receipt Inventory - Hospital'
                 description[data.frame == 'completion']                             <- 'Completion'
                 description[data.frame == 'discrepancies']                          <- 'Discrepancies'
                 description[data.frame == 'med.hist.problems.scoring']              <- 'Filtered (thrombosis/embolism/pulmonary) subset of Medical History - Medical Problems'
                 description[data.frame == 'likely.diagnosis']                       <- 'PE most likely diagnosis (from xls/PE likely diagnosis.xlsx by s.goodacre@sheffield.ac.uk)'
                 description[data.frame == 'questionnaire.contact']                  <- 'Questionnaire Contact (for 30-day follow-up)'
                 description[data.frame == 'sign.off']                               <- 'Sign Off'
                 description[data.frame == 'case.review1']                           <- 'Case Review 1'
                 description[data.frame == 'case.review1.investigation']             <- 'Case Review 1 assessment of patients investigations.'
                 description[data.frame == 'case.review2']                           <- 'Case Review 2'
                 description[data.frame == 'case.review2.investigation']             <- 'Case Review 2 assessment of patients investigations.'
                 description[data.frame == 'case.review3']                           <- 'Case Review 3'
                 description[data.frame == 'case.review3.investigation']             <- 'Case Review 3 assessment of patients investigations.'
                 description[data.frame == 'case.review']                            <- 'Merged Case Reviews from reviewers 1, 2 and 3'
                 description[data.frame == 'biomarker_exclusions_20170223_raw']      <- 'Individuals on anticoagulants to be excluded from analyses 20170223 (RAW)'
                 description[data.frame == 'biomarker_exclusions_20170223_clean']    <- 'Individuals on anticoagulants to be excluded from analyses 20170223 (CLEAN)'
                 description[data.frame == 'biomarker_exclusions_clean']             <- 'Individuals on anticoagulants to be excluded from analyses 2017 (CLEANER VERSION OF biomarkers_exclusions_20170223_clean)'
                 description[data.frame == 'cdr.active.medical.comorbidities.raw']   <- 'Individuals identified as having active medical comorbities that meet the Delphi Consensus Clinical Decision Rules (RAW)'
                 description[data.frame == 'cdr.active.medical.comorbidities.clean'] <- 'Individuals identified as having active medical comorbities that meet the Delphi Consensus Clinical Decision Rules (CLEAN)'
                 description[data.frame == 'cdr.obstetric.complications.raw']        <- 'Individuals identified as having obstetric complications that meet the Delphi Consensus Clinical Decision Rules (RAW)'
                 description[data.frame == 'cdr.obstetric.complications.clean']      <- 'Individuals identified as having obstetric complications that meet the Delphi Consensus Clinical Decision Rules (CLEAN)'
                 description[data.frame == 'cdr.supplementary']                      <- 'Combined list of individuals meeting the active medical comorbities or obstetric complications for the Delphi Consensus Clinical Decision Rules.'
                 description[data.frame == 'missing']                                <- 'Individuals with an event date but no other data'
                 description[data.frame == 'ecg.pe.classification.raw']              <- 'Classification of ECGs showing relevance to Pulmonary Embolism (provided as XLS by s.goodacre@sheffield.ac.uk)'
                 description[data.frame == 'xray.pe.classification.raw']             <- 'Classification of X-Rays showing relevance to Pulmonary Embolism (provided as XLS by s.goodacre@sheffield.ac.uk)'
                 description[data.frame == 'ukoss.exclusions']                       <- 'UKOSS exclusions'
})
