...
```{r results_biomarker_excl_anti_coag_troponin, echo = FALSE, cache = FALSE, warning = FALSE, message = FALSE, fig.width = 10, fig.height = 10, results = 'markup', eval = TRUE}
...
to.plot$scatter
## Summary Table
dplyr::filter(summary.biomarkers.excl.anti.coag$first.st.vte, Measurement == 'Troponin') %>%
    dplyr::select(-Measurement) %>%
        kable(caption = "Summary of Troponin (ng/ml) by Primary Classification")
t <- dplyr::filter(dipep, exclude.anti.coag == "No")
table(t$first.st.vte, t$troponin.cat) %>% kable(caption = "PE Status v Troponin (ng/ml) dichotomisation")
...
## Regression results
biomarker.excl.anti.coag$troponin$roc
biomarker.excl.anti.coag$troponin$fitted %>% summary()
## CAT2
## CAT
