```{r results_lasso_categorical, echo = FALSE, cache = FALSE, results = 'hide', eval = FALSE}
## Categorised variables
## Covariates
## covar <- dplyr::filter(dipep, group == 'Suspected PE') %>%
##          dplyr::select(age.cat, bmi.cat, smoking.cat, pregnancies.over.cat, pregnancies.under.cat,
...
##                        ## likely.diagnosis, d.dimer.cat,
##                        pe) %>%
##          dplyr::select(age.cat, smoking.cat, heart.rate.cat, pe) %>%
...
## status <- covar[,ncol(covar)]
## covar <- covar[,1:ncol(covar) - 1]
## LASSO
...
## lasso.categorical <- glmnet::glmnet(x      = covar,
##                             y      = status,
##                             family = 'binomial')
## cv.lasso.categorical <- glmnet::cv.glmnet(x      = covar,
##                                   y      = status,
##                                   family = 'binomial',
##                                   nfolds = length(status))
## cv.lasso.auc.categorical <- glmnet::cv.glmnet(x      = covar,
##                                   y      = status,
##                                   family = 'binomial',
##                                   type.measure = 'auc',
##                                   nfolds = length(status))
...
suspected.pe <- dplyr::filter(dipep, group == 'Suspected PE') %>%
                ## ToDo - Remove select statement, should work with whole data frame
                dplyr::select(pe, age.cat, bmi.cat, smoking.cat, pregnancies.over.cat, pregnancies.under.cat)
status <- nrow(suspected.pe)
##                             history.thrombosis, history.veins, history.iv.drug, thrombo, cesarean,
##                             injury, thrombosis, existing.medical, preg.post, trimester, multiple.preg,
##                             travel, immobil, this.pregnancy.problems, prev.preg.problem,
##                             presenting.features.pleuritic, presenting.features.non.pleuritic,
...
lasso.categorical <- glmnetUtils::glmnet(data = suspected.pe,
                                         .formula,
                                         family  = 'binomial')
cv.lasso.categorical <- glmnetUtils::cv.glmnet(data = suspected.pe,
                                              .formula,
                                              family  = 'binomial',
                                              nfolds = nrow(suspected.pe))
cv.lasso.auc.categorical <- glmnetUtils::cv.glmnet(data = suspected.pe,
                                              .formula,
                                              family  = 'binomial',
                                              type.measure = 'auc',
                                              nfolds = nrow(suspected.pe))
...
```{r results_lasso_continuous, echo = FALSE, cache = FALSE, results = 'hide', eval = FALSE}
## Continuous variables
## Covariates
## covar <- dplyr::filter(dipep, group == 'Suspected PE') %>%
##          dplyr::select(age, bmi, smoking, pregnancies.over, pregnancies.under,
##                        history.thrombosis, history.veins, history.iv.drug, thrombo, cesarean,
##                        injury, thrombosis, existing.medical, preg.post, trimester, multiple.preg,
##                        travel, immobil, this.pregnancy.problems, prev.preg.problem,
##                        presenting.features.pleuritic, presenting.features.non.pleuritic,
...
##                        presenting.features.haemoptysis, presenting.features.cough,
##                        presenting.features.syncope, presenting.features.palpitations,
##                        presenting.features.other, respiratory.rate,
##                        o2.saturation, bp.systolic, bp.diastolic, ecg.cat, xray.cat,
##                        ## likely.diagnosis,
##                        d.dimer, heart.rate,
##                        pe) %>%
##          dplyr::select(age, smoking, pe) %>%
##          mutate(use = complete.cases(.)) %>%
##          filter(use == TRUE) %>%
##          dplyr::select(-use) %>%
##          data.matrix()
...
##                                  family = 'binomial',
##                                  nfolds = length(status))
## cv.lasso.auc.continuous <- cv.glmnet(x      = covar,
##                                  y      = status,
##                                  family = 'binomial',
##                                  type.measure = 'auc',
##                                  nfolds = length(status))
...
#################################################
.formula <- reformulate(response = 'pe',
                        termlabels = c('age', 'bmi', 'smoking', 'pregnancies.over', 'pregnancies.under',
                                        'history.thrombosis', 'history.veins', 'history.iv.drug', 'thrombo', 'cesarean',
                                       'injury', 'thrombosis', 'existing.medical', 'preg.post', 'trimester', 'multiple.preg',
                                        'travel', 'immobil', 'this.pregnancy.problems', 'prev.preg.problem',
                                        'presenting.features.pleuritic', 'presenting.features.non.pleuritic',
                                        ...
                                        'presenting.features.haemoptysis', 'presenting.features.cough',
                                        'presenting.features.syncope', 'presenting.features.palpitations',
                                        'presenting.features.other', 'respiratory.rate', 'heart.rate',
                                        'o2.saturation', 'bp.systolic', 'bp.diastolic', 'ecg', 'xray'))

suspected.pe <- dplyr::filter(dipep, group == 'Suspected PE') ## %>%
                ## ToDo - Remove select statement, should work with whole data frame
                ## dplyr::select(pe, age, bmi, smoking, pregnancies.over, pregnancies.under)
status <- nrow(suspected.pe)

lasso.continuous <- glmnetUtils::glmnet(data = suspected.pe,
                                         .formula,
                                         family  = 'binomial')
cv.lasso.continuous <- glmnetUtils::cv.glmnet(data = suspected.pe,
                                              .formula,
                                              family  = 'binomial',
                                              nfolds = nrow(suspected.pe))
cv.lasso.auc.continuous <- glmnetUtils::cv.glmnet(data = suspected.pe,
                                              .formula,
                                              family  = 'binomial',
                                              type.measure = 'auc',
                                              nfolds = nrow(suspected.pe))
...
```{r results_lasso_continuous_roc, echo = FALSE, cache = FALSE, results = 'asis', eval = FALSE}
## Obtain minimum values
lambda.1se <- which(cv.lasso.continuous$lambda == cv.lasso.continuous$lambda.1se)
lambda.min <- which(cv.lasso.continuous$lambda == cv.lasso.continuous$lambda.min)
## Extract coefficients for the corresponding lambda
beta <- lasso.continuous[,lambda.min]
## ToDo - Augment ROC plots with the AUC, see http://blog.revolutionanalytics.com/2016/11/calculating-auc.html
