...
```{r results_biomarker_all_sensitivity_specificity_prep, echo = FALSE, cache = FALSE, warning = FALSE, message = FALSE, fig.width = 10, fig.height = 10, results = 'markup', eval = TRUE}
...
theshold <- 0.1
summary.stats <- list()
summary.stats$aptt <- dipep_roc(biomarker.all$predicted,
                                to.plot = c('aptt'),
                                title    = 'APTT biomarker performance',
                                threshold = threshold,
                                rocr      = 0.95)
## BNP
summary.stats$bnp <- dipep_roc(biomarker.all$predicted,
                                to.plot = c('bnp'),
                                title    = 'BNP biomarker performance',
                                threshold = threshold,
                                rocr      = 0.95)
## CLAUSS.FIBRINOGEN
summary.stats$clauss.fibrinogen <- dipep_roc(biomarker.all$predicted,
                                to.plot = c('clauss.fibrinogen'),
                                title    = 'Clauss Fibrinogen biomarker performance',
                                threshold = threshold,
                                rocr      = 0.95)
## DDIMER.ELISA
summary.stats$ddimer.elisa <- dipep_roc(biomarker.all$predicted,
                                to.plot = c('ddimer.elisa'),
                                title    = 'D-Dimer ELISA biomarker performance',
                                threshold = threshold,
                                rocr      = 0.95)
## DDIMER.INNOVANCE
summary.stats$ddimer.innovance <- dipep_roc(biomarker.all$predicted,
                                to.plot = c('ddimer.innovance'),
                                title    = 'D-Dimer Innovance biomarker performance',
                                threshold = threshold,
                                rocr      = 0.95)
## MRPROANP
summary.stats$mrproanp <- dipep_roc(biomarker.all$predicted,
                                to.plot = c('mrproanp'),
                                title    = 'MRProANP biomarker performance',
                                threshold = threshold,
                                rocr      = 0.95)
## PLASMIN.ANTIPLASMIN
summary.stats$plasmin.antiplasmin <- dipep_roc(biomarker.all$predicted,
                                to.plot = c('plasmin.antiplasmin'),
                                title    = 'Plasmin.Antiplasmin biomarker performance',
                                threshold = threshold,
                                rocr      = 0.95)
## PROTHOMBIN.TIME
summary.stats$prothombin.time <- dipep_roc(biomarker.all$predicted,
                                to.plot = c('prothombin.time'),
                                title    = 'Prothombin Time biomarker performance',
                                threshold = threshold,
                                rocr      = 0.95)
## PROTHROMBIN.FRAGMENT
summary.stats$prothrombin.fragments <- dipep_roc(biomarker.all$predicted,
                                to.plot = c('prothrombin.fragments'),
                                title    = 'Prothrombin Fragment biomarker performance',
                                threshold = threshold,
                                rocr      = 0.95)
## THROMBIN.GENERATION.ENDOGENOUS.POTENTIAL
summary.stats$thrombin.generation.endogenous.potential <- dipep_roc(biomarker.all$predicted,
...
summary.stats$all <- rbind(summary.stats$aptt$summary.stats,
    ...
    arrange(Term) %>%
    kable(digits = 3,
          caption = 'Summary Statistics for Continuous Biomarkers for a minimum sensitivity of 95%')
