...
continuous <- dipep_rpart(df             = dipep,
...
plotcp(continuous$rpart.full)

```

The Complexity Parameter (CP) at the step/number of splits (`r continuous$rpart.full.splits.min`) that corresponds to the minimum cross-validated error is `r continuous$rpart.full.cp.min`.

```{r results_rpart_continuous_prune, echo = FALSE, cache = FALSE, fig_width = 15, fig_height = 10, results = 'rmarkdown', eval = TRUE}
prp(continuous$pruned.min,
    type        = prp.opts$type,
    extra       = prp.opts$extra,
    box.palette = prp.opts$box.palette,
    yesno       = prp.opts$yesno,
...
continuous$roc.all$plot
continuous$roc.all$summary.stats %>%
