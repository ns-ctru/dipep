...
logistic$medical.complication <- dipep_glm(df = dipep,
                                           classification = 'first.st',
                                           predictor = 'medical.complication',
                                           model = 'Medical Complications (Delphi reviewed)',
                    exclude = NULL,
                    exclude.dvt = TRUE,
                    exclude.non.recruited = TRUE,
                    exclude.missing       = FALSE)
## Delphi - Obstetric Complication
logistic$obstetric.complication <- dipep_glm(df = dipep,
                                           classification = 'first.st',
                                           predictor = 'obstetric.complication',
                                           model = 'Obstetric Complications (Delphi reviewed)',
                    exclude = NULL,
                    exclude.dvt = TRUE,
                    exclude.non.recruited = TRUE,
                    exclude.missing       = FALSE)

logistic$medical.complication$tidied
logistic$obstetric.complication$tidied
...
logistic$glance <- rbind(logistic$age$glance,
                         ...
                         logistic$medical.complication$glance,
                         logistic$obstetric.complication$glance)
...
logistic$predicted <- rbind(logistic$age$predicted,
                            ...
                            logistic$medical.complication$predicted,
                            logistic$obstetric.complication$predicted)
...
logistic$cross.tab <- rbind(logistic$age.cat$table,
                            ...
                            logistic$medical.complication$table,
                            logistic$obstetric.complication$table)
## Saturated
...
## Select the results from logistic regression of Existing scores for combininig
to.sum.delphi <- dplyr::filter(existing$tidied, term != '(Intercept)') %>%
                 dplyr::filter(term %in% c('delphi.medical.historyTRUE',
                                           'delphi.medical.complicationTRUE',
                                           'delphi.obstetric.complicationTRUE',
                                           'delphi.gestationTRUE')) %>%
                 dplyr::select(term, estimate, lci, uci, p.value)
to.sum.simplified <- dplyr::filter(existing$tidied, term != '(Intercept)') %>%
                     dplyr::filter(term %in% c('simplified.neoplasm',
                                               'simplified.lower.limb.unilateral.pain',
                                               'simplified.pain.palpitations')) %>%
                     dplyr::select(term, estimate, lci, uci, p.value)
to.sum.wells <- dplyr::filter(existing$tidied, term != '(Intercept)') %>%
                     dplyr::filter(term %in% c('wells.alternative.permissive',
                                               'wells.alternative.strict')) %>%
                     dplyr::select(term, estimate, lci, uci, p.value)
to.sum.perc <- dplyr::filter(existing$tidied, term != '(Intercept)') %>%
                     dplyr::filter(term %in% c('perc.hormone',
                                               'perc.leg.swelling')) %>%
                     dplyr::select(term, estimate, lci, uci, p.value)
to.sum <- rbind(to.sum,
                to.sum.delphi,
