...
logistic$predicted <- merge(logistic$age$predicted,
    ...
    ## Age is complete, use observed values from that, remove all others
                      dplyr::select(pe.age, pred.age, pred.age.cat, pred.smoking,
                                    ...
                                    pred.presenting.features.other, pred.surgery,
                                    pred.history.thrombosis, pred.history.veins, ## pred.history.iv.drug,
                                    pred.trimester, pred.this.pregnancy.problems,
                                    pred.thrombo, pred.multiple.preg, pred.travel, pred.immobil,
                                    pred.ecg.cat, pred.ecg, pred.xray.cat, pred.xray)
## Sort out names
...
logistic$saturated <- dipep_glm(df = dipep,
                                predictor = c('age', 'bmi', 'temperature', 'bp.diastolic', 'bp.systolic',
                                              'o2.saturation', 'respiratory.rate', 'smoking',
...
ggplot(dplyr::filter(to.plot, name %in% c('history.thrombosis',
                                          'history.veins',
                                          'thrombo',
                                          'surgery',
                                          'thrombosis')),
