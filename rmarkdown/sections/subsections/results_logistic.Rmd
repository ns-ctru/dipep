...
logistic$surgery <- dipep_glm(df = dipep,
                              classification = 'first.st',
                              predictor = 'surgery',
                              model = 'Surgery in last 4 weeks',
                    exclude = NULL,
                    exclude.dvt = TRUE,
                    exclude.non.recruited = TRUE,
                    exclude.missing       = FALSE)
## Cesarean
logistic$cesarean <- dipep_glm(df = dipep,
                              classification = 'first.st',
                              predictor = 'cesarean',
                              model = 'Cesarean',
                    exclude = NULL,
...
logistic$thromb.event <- dipep_glm(df = dipep,
                    ...
                    exclude.non.recruited = TRUE,
                    exclude.missing       = FALSE)
## Obstetric Complications (VTE-related)
logistic$obstetric.complication.vte  <- dipep_glm(df = dipep,
                                    classification = 'first.st',
                                    predictor = 'obstetric.complications',
                                    model = 'Other Problem with this pregnancy (VTE-related)',
                    exclude = NULL,
                    exclude.dvt = TRUE,
                    exclude.non.recruited = TRUE,
                    exclude.missing       = FALSE)
...
logistic$tidied <- rbind(logistic$age$tidied,
                         ...
                         logistic$obstetric.complication.vte$tidied,
...
logistic$glance <- rbind(logistic$age$glance,
                         ...
                         logistic$obstetric.complication.vte$glance,
...
logistic$predicted <- rbind(logistic$age$predicted,
                            ...
                            logistic$obstetric.complication.vte$predicted,
...
logistic$cross.tab <- rbind(logistic$age.cat$table,
                            ...
                            logistic$obstetric.complication.vte$table,
...
to.sum.ddimer.all <- dplyr::filter(biomarker.all$tidied, term != '(Intercept)') %>%
                     dplyr::filter(term %in% c('D-Dimer (Hospital Gestation Specific) : Abnormal',
...
to.sum <- dplyr::filter(logistic$tidied, term != '(Intercept)') %>%
          dplyr::select(term, estimate, lci, uci, p.value, or, or.lci, or.uci)
## Select the results from logistic regression of Existing scores for combininig
to.sum.delphi <- dplyr::filter(existing$tidied, term != '(Intercept)') %>%
                 dplyr::filter(term %in% c('Delphi : Medical History (Hospital Admission, Surgery or Injury)',
                                           'Delphi : Surgery (excl. Cesarean)',
                 ...
                 mutate(term = gsub('Medical History (Hospital Admission, Surgery or Injury)', 'Hospital Admission, Surgery or Injury', term))
to.sum.simplified <- dplyr::filter(existing$tidied, term != '(Intercept)') %>%
                     dplyr::filter(term %in% c('Geneva : Neoplasm',
                                               'Geneva : Unilateral lower limb pain',
                                               'Geneva : Pain on limb palpitation')) %>%
...
to.sum.ddimer <- rbind(to.sum.ddimer.all,
                       to.sum.ddimer.excl.anti.coag) %>%
    mutate(or = ifelse(term == 'D-Dimer (Hospital) : Abnormal - Excluding those on Anti-Coagulants',
                       yes = NA,
                       no  = or),
           or.lci = ifelse(term == 'D-Dimer (Hospital) : Abnormal - Excluding those on Anti-Coagulants',
                       yes = NA,
                       no  = or.lci),
           or.uci = ifelse(term == 'D-Dimer (Hospital) : Abnormal - Excluding those on Anti-Coagulants',
                       yes = NA,
                       no  = or.uci))
rm(to.sum.ddimer.all, to.sum.ddimer.excl.anti.coag)
...
kable(to.sum,
      ...
      caption = 'Point estimates and 95% CIs, OR and 95% CIs and p-values for Univariable Regression')
rm(to.sum)
...
names(to.sum.ddimer.all) <- gsub('VTE', 'PE', names(to.sum.ddimer.all))
to.sum.ddimer.excl.anti.coag <- rbind(biomarker.excl.anti.coag$d.dimer.cat$table,
                     ...
                     mutate(predictor = paste0(predictor, ' - Excluding those on Anti-Coagulants'))
names(to.sum.ddimer.excl.anti.coag) <- gsub('VTE', 'PE', names(to.sum.ddimer.excl.anti.coag))
to.tab <- rbind(to.sum,
                to.sum.existing,
                to.sum.ddimer.all,
                to.sum.ddimer.excl.anti.coag) %>%
...
## Generate but don't plot D-Dimer Hospital
biomarker.all$roc.d.dimer <- dipep_roc(df = biomarker.all$predicted,
                                       to.plot = c('d.dimer.cat',
                                                   'd.dimer.gestation.cat'),
                                       threshold = 0.3,
                                       title = 'D-Dimer (Hospital) - All')
biomarker.all$roc.d.dimer$summary.stats <- biomarker.all$roc.d.dimer$summary.stats %>%
        mutate(Term = gsub('D-Dimer (Hospital) : Dichotomised', 'D-Dimer (Hospital) : All', Term),
           Term = gsub('D-Dimer (Hospital) : Dichotomised (Gestation Specific)', 'D-Dimer (Hospital Gestation Specific) : All', Term))
biomarker.excl.anti.coag$roc.d.dimer <- dipep_roc(df = biomarker.excl.anti.coag$predicted,
                                                  to.plot = c('d.dimer.cat',
                                                              'd.dimer.gestation.cat'),
                                                  threshold = 0.1,
                                                  title = 'D-Dimer (Hospital) - Excluding those on anti-coagulants')
biomarker.excl.anti.coag$roc.d.dimer$summary.stats <- biomarker.excl.anti.coag$roc.d.dimer$summary.stats %>%
    mutate(Term = gsub('D-Dimer (Hospital) : Dichotomised', 'D-Dimer (Hospital) : Excluding those on Anti-coagulants', Term),
           Term = gsub('D-Dimer (Hospital) : Dichotomised (Gestation Specific)', 'D-Dimer (Hospital Gestation Specific) : Excluding those on Anti-coagulants', Term))
...
rbind(logistic$roc.physiology$summary.stats,
      ...
      biomarker.all$roc.d.dimer$summary.stats,
      biomarker.excl.anti.coag$roc.d.dimer$summary.stats) %>%
    arrange(Term) %>%
    ## dplyr::select(`-PPV`, `-NPV`) %>%
    kable(digits  = 3,
          caption = 'Summary Statistics for Univariable logistic regression')
