...
```{r results_rpart_categorical_prune, echo = FALSE, cache = FALSE, results = 'rmarkdown', fig_width = 10, fig_height = 10, eval = TRUE}
cp <- 0.0084270
...
pred.obs <- cbind(.predict[,2] %>% as.vector(), categorical_prune$y)
...
```{r results_rpart_continuous_prune, echo = FALSE, cache = FALSE, fig_width = 10, fig_height = 10, results = 'rmarkdown', eval = TRUE}
cp <- 0.016854
...
```{r results_rpart_continuous_roc, echo = FALSE, cache = FALSE, fig_width = 10, fig_height = 10, warnings = FALSE, eval = FALSE}
.predict <- predict(continuous_prune, type = 'prob')
.predicted <- prediction(.predict[,2] %>% as.vector(), continuous_prune$y)
pred.obs <- cbind(.predict[,2] %>% as.vector(), continuous_prune$y)
names(pred.obs) <- c('predicted', 'obs')
ggplot(pred.obs, aes(d = pe, m = predicted)) +
    geom_roc(n.cuts = 10) + style_roc() +
