...
```{r results_existing_delphi_primary, echo = FALSE, cache = FALSE, results = 'markdown', eval = TRUE}
ggplot(dplyr::filter(dipep, !is.na(pe)), aes(x = delphi.primary)) +
    geom_bar(aes(fill = pe), position = 'dodge') +
...
```{r results_existing_delphi_primary_accuracy, echo = FALSE, cache = FALSE, results = 'markdown', eval = TRUE}
